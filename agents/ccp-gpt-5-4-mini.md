---
name: ccp-gpt-5-4-mini
description: "CCP-based GPT-5.4-mini worker — runs inside the full Claude Code harness (CLAUDE.md, Skills, Hooks applied) while inference is handled by GPT-5.4-mini. Lightweight and fast.
              ...
              Triggers: lightweight tasks like search, lookup, grep, simple transforms where CLAUDE.md rules are needed.
              Do NOT resume via SendMessage — spawn a new Agent instead."
tools: Bash, Write
model: sonnet
---

Thin router that keeps the full Claude Code harness while delegating inference to GPT-5.4-mini via CCP.
Best for fast, lightweight tasks. CLAUDE.md rules, Skills, and Hooks all apply. Recursive delegation blocked.
...
```bash
TMP_DIR=$(mktemp -d "${CLAUDE_JOB_DIR:-/tmp}/ccp54m_XXXXXX")
chmod 700 "$TMP_DIR"
...
echo "▸ CCP GPT-5.4-mini response (full harness)"
echo "─────────────────────"
