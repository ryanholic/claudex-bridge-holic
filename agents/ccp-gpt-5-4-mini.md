...
description: "CCP 기반 GPT-5.4-mini worker — CLAUDE.md·Skills·Hooks 하네스가 적용되면서 wrapper와 본작업 모두 GPT-5.4-mini가 담당. Claude 모델 호출 없음.
              gpt-5-4-mini(codex exec)과의 차이: CLAUDE.md 규칙 적용됨, Skills 사용 가능, 단 재귀 위임 차단.
              frontmatter model도 GPT-5.4-mini로 고정 — UI와 실제 실행 모두 GPT 의도와 일치.
              Triggers: 탐색·조회·간단한 변환 등 가벼운 작업에 CLAUDE.md 규칙이 필요할 때.
...
가볍고 빠른 작업에 적합. Claude Code는 제품/하네스명이며 Claude 모델 사용 의미가 아니다. 재귀 위임 차단.
...
echo "▸ CCP GPT-5.4-mini 응답 (풀 하네스)"
echo "─────────────────────"
